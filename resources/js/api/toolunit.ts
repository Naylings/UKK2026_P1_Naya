...
export const toolUnitApi = {
  ...
  list: async (params?: {
    ...
    const res = await apiClient.get<PaginatedToolUnitsResponse>("/tool-units", {
      params,
    });
    return res.data;
  },
  ...
  get: async (code: string): Promise<ToolUnit> => {
    const res = await apiClient.get<ToolUnitResponse>(`/tool-units/${code}`);
   ...
   */
  getConditionHistory: async (code: string): Promise<ConditionHistoryResponse> => {
